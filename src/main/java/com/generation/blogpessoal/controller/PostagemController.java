...
@CrossOrigin(origins = "*", allowedHeaders = "*") // Em um ambiente de produção real, substituiríamos o asterisco ("*")
													// pelo endereço do servidor front-end.
...
//			ResponseEntity.notFound().build();
		...
		return postagemRepository.findById(id).map(resposta -> ResponseEntity.ok(resposta))
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build()); // SELECT * FROM tb_postagens WHERE id =
																				// ?;
	}
